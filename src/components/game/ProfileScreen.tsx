...
import { ArrowLeft, TrendingUp, History, Wallet, Trophy } from 'lucide-react';
...
export function ProfileScreen({ user, onBack, onDeposit }: ProfileScreenProps) {
      ...
      <div className="mx-3 mt-3 rounded-xl bg-card p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button className="rounded-lg bg-secondary px-4 py-2 text-xs font-bold text-foreground">
              Withdraw
            </button>
          </div>
        </div>
              ...
              <div key={tx.id} className="flex items-center justify-between rounded-lg bg-card p-3">
                <div>
                  <span className={`text-xs font-bold ${
                    ...
                    tx.type === 'deposit' ? 'text-primary' : 'text-muted-foreground'
                  }`}>
                    {tx.type.toUpperCase()}
                  </span>
          ...
          <div className="space-y-2">
            ...
            {deposits.map(tx => (
              <div key={tx.id} className="rounded-lg bg-card p-3">
                ...
                {tx.tx_hash && (
                  <p className="mt-1 text-[10px] text-muted-foreground font-mono truncate">{tx.tx_hash}</p>
                )}
